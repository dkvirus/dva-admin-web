...
import { arrayToTree, queryArray, filterMenus, config } from '../../utils/index';

const Menus = ({ siderFold, user, darkTheme, location, handleClickNavMenu, navOpenKeys, changeOpenKeys, menu }) => {
  // 获取当前登录用户拥有的权限数组
  const { authorities } = user;
  // 根据权限数组过滤菜单 ========================= TODO 待完善权限功能
  if (config.isPermisson) {
      menu = filterMenus(menu, authorities);
  }
  // 生成树状
